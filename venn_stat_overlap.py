...
import pandas as pd
abd = 'D:/desk/file_bak/XMSH_202311_7705/Abundance.filtered.xls'

df = pd.read_csv(abd,sep='\t',index_col='Species')
df = df.iloc[:,6:]
writer = pd.ExcelWriter('D:/desk/file_bak/XMSH_202311_7705/venn_stat.xlsx', engine='xlsxwriter')
for i in range(1,32):
    s1 = 'C' + str(i)
    s2 = 'T' + str(i)
    ...
    for j in gongyou:
        ...
        abd1 = df.loc[j,s1]
        abd2 = df.loc[j,s2]
        temp_dict['common'+ ' in ' + s1][name] = abd1
        temp_dict['common'+ ' in ' + s2][name] = abd2
