...
group_lst <- c('H_FJ__MC','H_NFC__MC','L_FJ__MC','L_NFC__MC','M_FJ__MC','M_NFC__MC','NC__MC','PC__MC')


#gp <- group_lst[1]
for (gp in group_lst ) {
  mic_prefix <- 'D:/desk/XMSH_202309_6289/files/20230517_DZQD2023040472-b1_张玉刚-陈奕州-27个小鼠2bRAD-M项目_结题报告/Report/02.Community_Structure'
  met_f <- '代谢物通路汇总表.xlsx'
  met_prefix <- 'D:/desk/XMSH_202309_6289/files/DZQD2023040473-售后2-2023.5.31/DZQD2023040473-售后2-2023.5.30/DZQD2023040473-张玉刚-陈奕州-非靶向代谢-LCMS-EMDB(项目报告)/7.代谢通路分析'
  mic_f <- 'species.xls'
  out <- paste0(gp,"_",'circle.pdf')
  mic_f <- paste0(mic_prefix,'/',gp,'/relative_abundance/',mic_f)
  met_f <- paste0(gp,'/all/',met_f)
  ...
  #
  #
  pdf(file = out,height = 9,width = 16)
  circos.clear()
  circos.par(circle.margin = c(0.2,1.5,0.6,0.6))
  ############ test
  # 画轮廓
