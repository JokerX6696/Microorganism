rm(list=ls())
setwd('D:/desk/XMSH_202312_8327/2.alpha_beta')
library('ggplot2')
library('ggpubr')
...
avg <- c(mean(df$Shannon[1:3]),mean(df$Shannon[4:6]),mean(df$Shannon[7:9]))
errbar = c(sd(df$Shannon[1:3]),sd(df$Shannon[4:6]),sd(df$Shannon[7:9]))

df_avg <- data.frame(Group=unique(g),avg=c(mean(df$Shannon[1:3]),mean(df$Shannon[4:6]),mean(df$Shannon[7:9])))
df_avg$err <- errbar
...
p <- ggplot() + 
  ...
  geom_signif(data=df_avg,
              aes(xmin='PBS', xmax='A5B5', annotations=pvalue, y_position=3.5),
