...
wilcox_test_zfx <- function(group1,group2){
  ...
  }else if(identical(unique(group1),unique(group2))){
    p_value <- 999
    lower_bound = 999
    upper_bound = 999
    median_difference = (lower_bound + upper_bound) / 2
...
selcet_sample_EtOH_4 <- group$Sample[group$Group == Sample2]
...
names(plot_df1) <- c(Sample,Sample2)
p_vet <- c()
...
levels(plot_df$Group)=c(Sample,Sample2)
levels(plot_df$L3) <- rev(rownames(plot_info_df))
#levels(plot_df$L3) <- rev(rownames(plot_info_df))
################
anno <- read.table('cog_info_TEST3.tsv',sep = '\t',quote = "")
names(anno) <- c('ID','anno')
nnnew <- c()
for (i in plot_df$L3) {
  a = anno[anno$ID == i,2]
  print(a)
  print(i)
  nnnew <- c(nnnew,anno[anno$ID == i,2])
}
nnnew <- gsub('"','',nnnew)
###############
p1 <- ggplot(plot_df,mapping=aes(x=L3,y=value,fill = Group),) +
  ...
  xlab("") +
  ylab("") +
  #scale_x_discrete(labels = rev(nnnew)) + 
...
p1 <- p1 + geom_bar(stat = "identity",position = position_dodge(1)) + 
  scale_fill_manual(breaks=c(Sample,Sample2),values = c("#ED0000FF","#00468BFF"))
########### p2
plot_df_2 <- plot_info_df[,4:6]
plot_df_2$L3 <- factor(rownames(plot_df_2),levels = levels(plot_df$L3))
plot_df_2$Group <- ifelse(plot_df_2$median < 0,Sample2,Sample)
...
ggsave(filename = paste0(Sample,'_vs_',Sample2,'_','stamp.png'),plot = p,device = 'png',width = 12,height = 6)
ggsave(filename = paste0(Sample,'_vs_',Sample2,'_','stamp.pdf'),plot = p,device = 'pdf',width = 12,height = 6)
sel <- group$Sample[group$Group == Sample | group$Group == Sample2]
t_df <- data[rownames(plot_info_df),sel]
wt_df <- data.frame(L3=rownames(plot_info_df),t_df,plot_info_df[,1:3])
wt_df <- wt_df[,-c(ncol(wt_df)-1,ncol(wt_df)-2)]
write.table(x = wt_df,file = paste0(Sample,'_vs_',Sample2,'_','stat.xls'),sep = '\t',row.names = FALSE,quote = F)
